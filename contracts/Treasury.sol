...
contract InubisTreasury is InubisAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for INKH
    ...
    function deposit(
        ...
        // mint INKH needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        INKH.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn INKH for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        INKH.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new INKH using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        INKH.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or INKH from the treasury, using sINKH as collateral. This might allow an
     * sINKH holder to provide INKH liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(INKH)) {
            require(permissions[STATUS.INKHDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sINKH.changeDebt(value, msg.sender, true);
        require(sINKH.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(INKH)) {
            INKH.mint(msg.sender, value);
            inkhDebt = inkhDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sINKH.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with INKH
     * @param _amount uint256
     */
    function repayDebtWithINKH(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.INKHDEBTOR][msg.sender],
            notApproved
        );
        INKH.burnFrom(msg.sender, _amount);
        sINKH.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        inkhDebt = inkhDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(INKH), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SINKH) {
            sINKH = IsINKH(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SINKH) {
            // 9
            sINKH = IsINKH(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(INKH.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns INKH valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(INKH)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return INKH.totalSupply() - inkhDebt;
    }
}
