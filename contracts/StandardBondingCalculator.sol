...
contract InubisBondingCalculator is IBondingCalculator {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(INKH)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(INKH), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(INKH)).decimals())).div(getTotalValue(_pair));
    }
}
