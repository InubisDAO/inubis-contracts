...
contract sInubis is IsINKH, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgINKH public gINKH; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked INKH", "sINKH", 9) ERC20Permit("Staked INKH") {
    ...
    function setgINKH(address _gINKH) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gINKH) == address(0), "gINKH:  gINKH already set");
        require(_gINKH != address(0), "gINKH:  gINKH is not a valid contract");
        gINKH = IgINKH(_gINKH);
    }
    ...
    /**
        @notice increases rINKH supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sINKH of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sINKH
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sINKH: insufficient balance");
    }
    ...
    // toG converts an sINKH balance to gINKH terms. gINKH is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gINKH.balanceTo(amount);
    }

    // fromG converts a gINKH balance to sINKH terms. sINKH is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gINKH.balanceFrom(amount);
    }

    // Staking contract holds excess sINKH
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply
                .sub(balanceOf(stakingContract))
                .add(gINKH.balanceFrom(IERC20(address(gINKH)).totalSupply()))
