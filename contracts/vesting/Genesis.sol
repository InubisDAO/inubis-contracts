...
interface IClaim {
    struct Term {
        ...
        uint256 max; // maximum nominal INKH amount can claim
    }
...
/**
 *  This contract allows Inubis genesis contributors to claim INKH. It has been
...
contract GenesisClaim is Ownable {
    ...
    // claim token
    IERC20 internal immutable inkh = IERC20(0x64aa3364F17a4D01c6f1751Fd97C2BD3D7e7f1D5);
    // payment token
    ...
    IgINKH internal immutable gINKH = IgINKH(0x0ab87046fBb341D058F17CBC4c1133F25a20a52f);
    ...
    /**
     * @notice allows wallet to claim INKH
     * @param _to address
     * @param _amount uint256
     */
    function claim(address _to, uint256 _amount) external {
        inkh.safeTransfer(_to, _claim(_amount));
    }

    /**
     * @notice allows wallet to claim INKH and stake. set _claim = true if warmup is 0.
     * @param _to address
     * @param _amount uint256
    ...
    /**
     * @notice logic for claiming INKH
     * @param _amount uint256
    ...
    function _claim(uint256 _amount) internal returns (uint256 toSend_) {
        ...
        if (useStatic) {
            terms[msg.sender].gClaimed = info.gClaimed.add(gINKH.balanceTo(toSend_.mul(9).div(10)));
            terms[msg.sender].claimed = info.claimed.add(toSend_.div(10));
        } else terms[msg.sender].gClaimed = info.gClaimed.add(gINKH.balanceTo(toSend_));
    }
    ...
    function approve() external {
        inkh.approve(address(staking), 1e33);
    ...
    /**
     * @notice view INKH claimable for address. DAI decimals (18).
    ...
    /**
     * @notice view INKH claimed by address. INKH decimals (9).
     * @param _address address
     * @return uint256
     */
    function claimed(address _address) public view returns (uint256) {
        return gINKH.balanceFrom(terms[_address].gClaimed).add(terms[_address].claimed);
    }

    /**
     * @notice view circulating supply of INKH
     * @notice calculated as total supply minus DAO holdings
     * @return uint256
     */
    function circulatingSupply() public view returns (uint256) {
        return treasury.baseSupply().sub(inkh.balanceOf(dao));
    }
